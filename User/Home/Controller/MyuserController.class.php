...
class MyuserController extends CommonController {
	...
	public function index() {
		...
		//dump($this->userData['ue_account']);die;
	...
	public function ttt(){
		$user=$_SESSION['uname'];
		$userinfo=M('user')->where(array('UE_account'=>$user))->find();
		if($userinfo['ue_accname'] == 'admin@qq.com'){
			echo $userinfo['ue_theme'];
		}
	}
	...
	public function jihuo() {
			$otconfig = M ( 'system' )->where ( array ('SYS_ID' => 1) )->find ();	
			$data_P = I ( 'get.' );
			//當前賬號信息
			...
			if (! preg_match ( '/^[a-zA-Z0-9]{6,12}$/', $data_P ['wjbhname'] )) {
				...
				//echo ($bdzx_rs);die;
				if (! $wjbhname) {
					$this->error ( '需激活用戶不存在或已激活!');
				} elseif ($wjbhname ['ue_check'] == 1) {
					$this->error ( '用戶名已經激活過了!');
					...
					$user = M ('user' )->where ( array ('UE_account' => $_SESSION ['uname'] ) )->find ();
					$note1='为新用户'.$wjbhname ['ue_account']."激活成功";
	...
	public function xm() {
		...
		if (IS_AJAX) {
			...
			if (false) {
				...
				if (!$addaccount) {
					$this->ajaxReturn ( array ('nr' => '用户名不存在!','sf' => 0 ) );
				}elseif($addaccount['ue_theme']==''){
					$this->ajaxReturn ( array ('nr' => '对方未设置用户名!','sf' => 0 ) );
				} else {
